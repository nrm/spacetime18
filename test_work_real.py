...
if False:
    ...
    fig.add_subplot(1, 2, 1)
    image2 = tifffile.imread('layouts/layout_2021-08-16.tif')
    image2=np.where(np.abs(image2)>5000,0,image2)
    plt.imshow(image2[:-1,:,3]-image2[1:,:,3])
    #plt.hist(image2.flatten(), bins=1000)
    if False:
        fig.add_subplot(1, 2, 2)
        geo_tiff = GeoTiff('layouts/layout_2021-08-16.tif')
        array = geo_tiff.read()
        print(array.shape)
        #plt.hist(np.array(array).flatten(), bins=1000)
        plt.imshow(array[:,:,1])
        plt.show()
...
#image2[:,:,0]=np.median(image2,axis=2)

image2[:,:,0]=np.median(image2,axis=2)
#image2[1:,:,0]=image2[:-1,:,3]-image2[1:,:,3] #np.median(image2,axis=2)

#image2[0,:,0]=np.zeros(image2.shape[1])

med=np.median(image2[::mm,::mm,layer])
sp2=np.fft.fft2(image2[::mm,::mm,layer]-med)
im2=(image2[::mm,::mm,layer]-med)*1.0
ix=im2.shape[0]
...
def calc_for_mults(crop,lay,im,jm,bPlot=False):
    image1 = crop * 1.0
    image1[:,:,0] = np.median(crop,axis=2)
    image1[1:, :, 0] = image1[:-1, :, 0]-image1[1:, :, 0]
    image1[0, :, 0]=np.zeros(len(image1[0, :, 0]))
    im2 = lay[::im, ::jm] * 1.0

    im2[1:,:]=im2[:-1,:]-im2[1:,:] #np.median(image2,axis=2)
    ...
    im1[ix[0] // 2:ix[0] // 2 + i1mx, ix[1] // 2:ix[1] // 2 + i1my] = image1[:, :, layer] * 1.0 - med1
    if bPlot:
        fig = plt.figure()
        fig.add_subplot(1, 2, 1)
    ...
    if bPlot:
        ...
        plt.imshow(im2__)
        plt.show()
        ccf_repro_images(image1[:, :, layer] * 1.0 - med1, im2__, 16)
    return snr
